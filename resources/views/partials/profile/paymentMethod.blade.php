<div class="d-flex justify-content-between align-items-center pb-3">
  ...
  <div>
    ...
    <div>
      <button type="button" class="btn btn-link a_link pl-4" id="profile-edit-payment-method-button-{{$loop}}"
        data-toggle="modal" data-target="#modalDeleteCreditCardForm{{$loop}}">
        Delete
      </button>
    </div>
  </div>
        ...
        <div class="modal-body">
          ...
          <form action="{{route('profile.credit_card')}}" method="post">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="ccNameEdit{{$loop}}" name="name"
                value="{{ old('name',$paymentMethod->credit_card->name) }}" required>
            </div>

            <div class="form-group">
              <label for="ccNumberEdit{{$loop}}">Card Number: </label>
              <input type="text" class="form-control" id="ccNumberEdit{{$loop}}" name="number"
                value="{{ old('number') }}" required>
            </div>

            <div class="form-group">
              <label for="ccDateEdit{{$loop}}">Expiry Date: </label>
              <input type="date" class="form-control" id="ccDateEdit{{$loop}}" name="expiration"
                value="{{ old('expiration',$paymentMethod->credit_card->expiration) }}" required>
            </div>

            <div class="form-group">
              <label for="ccCVVEdit{{$loop}}">CVC/CCV: </label>
              <input type="text" class="form-control" id="ccCVVEdit{{$loop}}" name="cvv" value="{{ old('cvv') }}"
                required>
            </div>
        </div>
  ...
  <!-- MODAL -->
  <div class="modal fade" id="modalDeleteCreditCardForm{{$loop}}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Are you sure you want to delete Credit Card info?</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-link a_link" data-dismiss="modal">Close</button>
          <button type="submit" class="btn button">Delete Credit Card</button>
        </div>
        </form>
      </div>
    </div>
...
<button type="button" class="btn button-secondary float-right" id="profile-edit-payment-method-button-{{$loop}}"
  ...
  <i class="fas fa-pen pr-2"></i>Edit
    ...
    <div>
      <button type="button" class="btn btn-link a_link pl-4" id="profile-edit-payment-method-button-{{$loop}}"
        data-toggle="modal" data-target="#modalDeletePaypalForm{{$loop}}">
        Delete
      </button>
    </div>
  </div>
...
<!-- MODAL -->
<div class="modal fade" id="modalEditPaypalForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
  aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        ...
        <form action="{{route('profile.paypal')}}" method="post">
          ...
          <div class="form-group">
            ...
            <input type="email" class="form-control" id="ppNameEdit{{$loop}}" name="email"
              value="{{ $paymentMethod->paypal->email }}" required>
          </div>
      </div>
  ...
  <!-- MODAL -->
  <div class="modal fade" id="modalDeletePaypalForm{{$loop}}" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Are you sure you want to delete Paypal info?</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
